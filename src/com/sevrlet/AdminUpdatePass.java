...
public class AdminUpdatePass extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		AdminSql edi = new AdminSql();
		
	    int n =edi.managerLogin(name, epassword);
		if(n==1){
			   if(newpsw!=null||(!(newpsw.equals("")))&&newpsw.equals(newpsw2)){
					AdminBeen admin = edi.getAdminByName(name);
					admin.setPassWord(newpsw);;
					if(edi.mupdatePassword(name, newpsw)){
						request.getSession().setAttribute("admin", admin);
						response.sendRedirect("manager/ManagerData.jsp");
					}else{
						response.sendRedirect("manager/Manager_xiugai.jsp");
					}
				}else{
					response.sendRedirect("manager/Manager_xiugai.jsp");
				}
